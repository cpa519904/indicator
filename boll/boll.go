...
func (this *Boll) sma(lines []define.Kline) float64 {
	s := len(lines)
	var sum float64 = 0
	for i := 0; i < s; i++ {
...
func (this *Boll) dma(lines []define.Kline, ma float64) float64 {
	s := len(lines)
	//log.Println(s)
	var sum float64 = 0
	for i := 0; i < s; i++ {
...
func (this *Boll) Boll(lines []define.Kline) (mid []float64, up []float64, low []float64) {
	l := len(lines)
